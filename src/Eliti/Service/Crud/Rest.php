...
abstract class Eliti_Service_Crud_Rest extends Eliti_Service_Crud
{
    ...
    public function getWhereRest($id = 0, $offset = 0, $orderBy = null, $WHERE_KEYWORDS, $limit = true, $filter = null)
    {
        $WHERE = "  WHERE 1 ";
        if ($id) {
    ...
    public function sort($newSort)
    {
        ...
        foreach ($newSort as $id) {
            $this->db->executar("UPDATE " . $this->getConfigTable() . " SET sort = {$total} WHERE id = {$id}");
            $total--;
        }
    }
}
