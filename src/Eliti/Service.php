...
class Eliti_Service
{
    ...
    {
        $str = is_string($param) ? trim($param) : $param;
    ...
    //    public function getParamDateObject($param) {
    //        $data = date("Y-m-d", strtotime($param));
    //        return Eliti_Date::isValid($data, "Y-m-d") ? $data : null;
    //    }
    ...
    {
        $array = is_array($param) ? $param : array();
        // corrige o problema da plugin do frontend object-to-formdata que traz array vazio como "[0] => "
        if (sizeof($array) === 1 && array_key_exists(0, $array) && !$array[0]) {
            return array();
        }
        return $array;
    }
