...
abstract class Eliti_Service_Crud extends Eliti_Service implements Eliti_Service_Interface
{
    ...
    public function delete($id, $filter)
    {
        ...
        // ele vai conectar e desconectar chamando o getById()
        $p = $this->getById($id, $filter);

        // daí conectar neste ponto para dar certo
        $this->db->conectar();
        if (!$p) {
    ...
    protected function saveOneToMany($idOne, $idsMany, $table, $columnOne, $columnMany)
    {
        // 1. pega id de todos os many postados
        $postedManyIds = [];
        foreach ($idsMany as $many) {
            $postedManyIds[] = (int)$many["id"];
        }

        // 2. pega id dos many atuais
        $result = $this->db->executar("SELECT $columnMany FROM $table WHERE $columnOne = $idOne");
        $savedManyIds = [];
        foreach ($result as $r) {
            $savedManyIds[] = (int)$r[$columnMany];
        }

        // 3. descobre quem será adicionado e adiciona
        $manyIdsToAdd = array_diff($postedManyIds, $savedManyIds);
        foreach ($manyIdsToAdd as $idMany) {
            $this->db->executar("INSERT INTO $table ($columnOne, $columnMany) VALUES ($idOne, $idMany)");
        }

        // 4. descobre quem será apagado e apaga
        $manyIdsToDelete = array_diff($savedManyIds, $postedManyIds);
        foreach ($manyIdsToDelete as $idMany) {
            $this->db->executar("DELETE FROM $table WHERE $columnOne = $idOne AND $columnMany = $idMany");
        }
    }
}
